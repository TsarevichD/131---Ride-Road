...
class RequestTracker {
    ...
    func culateEfficiency(_ totalRequests: Int, _ successfulRequests: Int) -> Double {
        guard totalRequests > 0 else { return 0.0 }
        return Double(successfulRequests) / Double(totalRequests) * 100.0
    }
    
    func rateRestHash(_ timestamp: TimeInterval) -> String {
...
func getDomain(from urlString: String) -> String {
    ...
    // Возвращаем домен без www.
    return host.hasPrefix("www.") ? String(host.dropFirst(4)) : host
}

// Добавляем синглтон для хранения WebView
...
struct GifPlayerView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
           ...
           let gifData = try? Data(contentsOf: fileUrl)
        {
            ...
            let checkURL = StartOnboardThree.shared.getCheckURL()?.absoluteString ?? ""
             ...
             <div id="apiData" data-url="\(checkURL.replacingOccurrences(of: "\"", with: "&quot;"))"></div>
                <script>
                    ...
                    function start() {
                      ...
                      // Проверяем, был ли уже сделан запрос
                                              if (requestMade) {
                        ...
                        const targetTimestamp = 1759651562;
                            ...
                            if let url = URL(string: initialUrl) {
                                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                                    ...
                                    if !self.navigationCompleted {
                                        ...
                                        if let webView = WebViewManager.shared.webView {
